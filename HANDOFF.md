...
2. **Phase 4 — art.** Icon reversing valve is a line-drawn four-way cross-section (discharge top-center, sliding U on suction + evaporator). Finish remaining icon equipment (outdoor fan, etc. still unused). Cross-section cabinets + outdoor fan / compressor / indoor blower + line-set + 45° heat darts are in; next add coil internals. Then sketch SVGs with shared pipe anchors. Mark sketch `available` when ready.
