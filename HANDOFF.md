...
| 4 | Icon / sketch / cross-section art swap | Partial. Tile **enabled**; default temporarily **cross-section** (restore **simple box** later). Abstract icon: schematic coils, compressor trapezoid, expansion triangles, line-drawn four-way RV cross-section. **Cross-section:** 2.5D outdoor/indoor cabinets, transparent **2/3** divider, outdoor axial fan, shortened scroll compressor + horizontal **RV** cylinder (compressor-wide, mid-way between compressor dome and cabinet top) + scaled **TXV** (raised, flared top cap), indoor cross-flow blower, temperature-colored **line-set** + **RV stub pipes** (see **Cross-section refrigerant routing** below), 45° heat-transfer darts. Coil internals and sketch art not started. Parked `iconLayout` / `reversingValveLayout` stay commented. |
...
- **Reversing valve on** (temporary default while polishing cross-section RV stubs; restore **off** later)
...
- Playback playing at **0.5×** (speed slider **0.125–1×**; half the prior range)
...
**System rows:**
1. **Type** (disabled) · **Cycle** (fire / snowflake) · **Reversing valve** (Off \| On; default **On** temporarily for cross-section stub polish)
...
**Cross-section cabinets & equipment:** Outdoor front **4:3** (`224×168`, floor on house floor `492`). Right-bay cutaway from **2/3** width: filled floor, **transparent** divider (`fill: none`; seam + dotted back edges remain), omitted right-side front vertical. Left bay: 4-blade axial fan. Right bay: scroll compressor (**25%** shorter than prior), horizontal **RV** cylinder (length = compressor width; diameter = TXV width; centered vertically between compressor dome and outdoor cabinet top), **TXV** vertical cylinder (**80%** scale, **`CROSS_SECTION_TXV_LIFT` `16`**, flared top ellipse with **2×** horizontal radius; base ellipse matches compressor-base aspect). Indoor: full-front cutaway cabinet + cross-flow blower (elliptical ends, arc ribs, scrolling slots). Equipment draw order keeps the **TXV stub behind the compressor**.

**Cross-section line-set:** Parallel pair (gap **`28`**). Top run meets the indoor cabinet **front top-right**; bottom run meets the TXV left side. Same `.pipe` color / width / dash / arrow HUD as simple-box. **Cooling:** top = **cool**, bottom = **cold**. **Heating:** top = **hot** into indoor (outdoor → indoor; path bounds flipped), bottom = **warm** out of indoor (indoor → outdoor). Solid-style chevrons on line-set **and stub pipes** (`.cross-section-stub-pipe`); dashed/arrow use global pipe styling.

**Cross-section refrigerant routing (RV on):** Temperature-colored stub `.pipe` paths with flow-ordered `d` and solid chevrons. Hidden when reversing valve off (`.hide-reversing-valve`). **Cooling:** TXV stub **warm** (outdoor coil → TXV); line-set U-turn **cool** (top line → RV top-right); compressor top **hot** → RV bottom; RV top-left **hot** → divider/outdoor coil; RV top-center **cool** → compressor left (U-turn). **Heating:** TXV stub **cold** (TXV → outdoor evaporator); line-set U-turn **hot** (→ RV top-right); compressor top **hot** → RV; RV top-left **cool** → outdoor coil; RV top-center **cool** → compressor left. RV–divider stub lands on the divider **back edge ∩ top-face front edge** (`outdoorFront.y`).

**Cross-section heat transfer:** 45° darts through each fan; labels hidden. Inbound ends on cabinet top far edge; outdoor outbound stops **`24`px above diagram bottom.
...
9. **Disabled HUD tiles** (Type) are reserved product surface — do not delete. Component style is enabled (simple box / icon / cross-section); sketch stays listed but skipped. Reversing valve is enabled; default **on** temporarily while cross-section stub pipes are polished (restore **off** later).
...
2. **Phase 4 — art.** Icon reversing valve is a line-drawn four-way cross-section (discharge top-center, sliding U on suction + evaporator). Finish remaining icon equipment (outdoor fan, etc. still unused). Cross-section: cabinets, fan, compressor, RV + stub pipes, TXV, line-set, heat darts in; **next:** coil internals. Then sketch SVGs with shared pipe anchors. Mark sketch `available` when ready. Restore defaults **simple box** + **RV off** when cross-section polish is done.
...
- Default presentation: heating, indoor right, light theme, house & weather, role-based coil labels, XL font, **dashed** lines, temperature-based color, **pressure-based** width & spacing, **heat transfer on**, **reversing valve on** (temporary for cross-section polish), playback **0.5×**.
