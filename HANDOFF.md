...
| 4 | Icon / sketch / cross-section art swap | Partial. Tile **enabled**; default **simple box**. Abstract icon shares the simple-box loop and paints schematic coils, compressor trapezoid, expansion triangles, and a line-drawn four-way reversing-valve cross-section. **Cross-section** is enabled and currently draws 2.5D outdoor + indoor cabinet outlines (front rectangle, top + right parallelograms); refrigerant art hidden in that style for now. Parked geometric units / `iconLayout` / `reversingValveLayout` stay commented. `sketch` stays skipped. |
...
2. **Phase 4 — art.** Icon reversing valve is a line-drawn four-way cross-section (discharge top-center, sliding U on suction + evaporator). Finish remaining icon equipment (outdoor fan, etc. still unused). Cross-section style is enabled with 2.5D outdoor + indoor cabinet outlines; next add internals / pipe anchors. Then sketch SVGs with shared pipe anchors. Mark sketch `available` when ready.
