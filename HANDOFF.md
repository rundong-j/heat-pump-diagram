...
`circuitLayout()` owns `loop` plus four colored `d` strings and box centers. **Hot** ends at the condenser Y; **warm** continues to the expansion valve; **cold** ends at the evaporator Y; **cool** continues to the compressor. Heating swaps which coil is condenser vs evaporator. The hidden `loop` path still traces the full rectangle for GSAP. Rebuild flow (particles **and** dashes) only when `topologyKey` changes (`showReversingValve`, `componentStyle`, `indoorSide`, `mode`, `overlays.heatTransfer`).
